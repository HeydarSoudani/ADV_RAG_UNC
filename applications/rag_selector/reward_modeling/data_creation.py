...
def create_training_data_v2(args):
    rag_methods = [
        # ('Qwen2.5-7B-Instruct', 'ircot'),
        # ('Qwen2.5-7B-Instruct', 'flare', 0.08),
        # ('Qwen2.5-7B-Instruct', 'dragin', 0.6),
    ...
    def _to_long(df: pd.DataFrame, methods=RAG_METHODS) -> pd.DataFrame:
        """Wide -> long. One row per (qid, query, method)."""
        rows = []
        for _, row in df.iterrows():
            for m in methods:
                if pd.isna(row[m]):
                    continue
                ans, em, conf, gens, sqs = row[m]
                rows.append({
                    "qid": row["qid"],
                    "query": row["query"],
                    "method": m,
    ...
    def _pair_frame(pos_row: pd.Series, neg_row: pd.Series) -> dict:
        """Create a single paired example dictionary."""
        return {
            # left/positive arm
            "qid_pos": pos_row["qid"],
            "query_pos": pos_row["query"],
            "method_pos": pos_row["method"],
            "answer_pos": pos_row["answer"],
            "em_pos": pos_row["em"],
            "conf_pos": pos_row["conf"],
            # right/negative arm
            "qid_neg": neg_row["qid"],
            "query_neg": neg_row["query"],
            "method_neg": neg_row["method"],
            "answer_neg": neg_row["answer"],
            "em_neg": neg_row["em"],
            "conf_neg": neg_row["conf"],
            # preference label: pos preferred over neg
            "pref": 1,
            # convenience flags
            "conf_relation": "pos>neg" if pos_row["conf"] > neg_row["conf"] else "pos<neg" if pos_row["conf"] < neg_row["conf"] else "pos=neg",
            "pair_type": None,  # will be set to "within" or "between"
        }
    
    # ---------------------------------------------------------------------
    # WITHIN-SAMPLE pairs (same qid, different RAG systems)
    #   - First build set2: conf(pos) < conf(neg)
    #   - Then sample an equal number from set1: conf(pos) > conf(neg)
    # ---------------------------------------------------------------------
    def build_within_pairs(df: pd.DataFrame, seed: int = 42):
        """
        Returns:
        within_lowpref: conf(pos) < conf(neg)
        within_highpref: conf(pos) > conf(neg), downsampled to match count of within_lowpref
        """
        rng = np.random.default_rng(seed)
        long_df = _to_long(df)

        # Split positive/negative by EM
        # We'll pair within the same qid across different methods.
        within_lowpref = []
        within_highpref = []

        for qid, g in long_df.groupby("qid"):
            pos = g[g["em"] == 1].copy()
            neg = g[g["em"] == 0].copy()
            if pos.empty or neg.empty:
                continue

            # Only pair across different methods
            # Cartesian product of pos x neg
            for _, pr in pos.iterrows():
                for _, nr in neg.iterrows():
                    if pr["method"] == nr["method"]:
                        continue
                    pair = _pair_frame(pr, nr)
                    pair["pair_type"] = "within"
                    if pr["conf"] < nr["conf"]:
                        within_lowpref.append(pair)
                    elif pr["conf"] > nr["conf"]:
                        within_highpref.append(pair)
                    # ties (==) are ignored

        within_lowpref_df = pd.DataFrame(within_lowpref)
        within_highpref_df = pd.DataFrame(within_highpref)

        # Shuffle
        if not within_lowpref_df.empty:
            within_lowpref_df = within_lowpref_df.sample(frac=1, random_state=seed).reset_index(drop=True)
        if not within_highpref_df.empty:
            within_highpref_df = within_highpref_df.sample(frac=1, random_state=seed).reset_index(drop=True)

        # Balance: keep all lowpref, and downsample highpref to equal size
        k = len(within_lowpref_df)
        if k == 0:
            balanced_highpref_df = within_highpref_df.iloc[0:0]  # empty
        else:
            balanced_highpref_df = within_highpref_df.head(k)

        return within_lowpref_df, balanced_highpref_df
    
    # ---------------------------------------------------------------------
    # BETWEEN-SAMPLE pairs (different qids and methods chosen freely)
    #   - You give n_per_set
    #   - We create:
    #       between_highpref: conf(pos) > conf(neg)
    #       between_lowpref:  conf(pos) < conf(neg)
    #   - Sampling is random but respects the confidence inequalities.
    # ---------------------------------------------------------------------
    def _sample_between_pairs(pos_df, neg_df, n, relation: str, seed: int):
        """
        relation: "gt" for conf(pos) > conf(neg), "lt" for conf(pos) < conf(neg)
        Returns list[dict] of size up to n.
        """
        rng = np.random.default_rng(seed)
        ...
        pos_df = pos_df.iloc[pos_idx].reset_index(drop=True)
        neg_df = neg_df.iloc[neg_idx].reset_index(drop=True)

        # We'll loop over positives and try to find a matching negative.
        # Allow re-use of negatives if needed; if you want strict uniqueness, keep a used set.
        out = []
        neg_arr = neg_df["conf"].to_numpy()

        # Pre-sort neg for quick mask selection per pos
        # We'll keep original rows in neg_df; we just select a random index from valid mask each time.
        for _, pr in pos_df.iterrows():
            if len(out) >= n:
                break

            if relation == "gt":
                valid = np.where(neg_arr < pr["conf"])[0]
            else:  # "lt"
                valid = np.where(neg_arr > pr["conf"])[0]
            ...
            nr = neg_df.iloc[j]
            pair = _pair_frame(pr, nr)
            pair["pair_type"] = "between"
            # Ensure strict inequality label is consistent
            if relation == "gt" and pair["conf_pos"] <= pair["conf_neg"]:
                continue
            if relation == "lt" and pair["conf_pos"] >= pair["conf_neg"]:
                continue
            out.append(pair)

        return out

    def build_between_pairs(df: pd.DataFrame, n_per_set: int, seed: int = 42):
        """
        n_per_set: how many examples you want in EACH set
                (we produce n_per_set with conf(pos) > conf(neg) and n_per_set with conf(pos) < conf(neg))
        Returns:
        between_highpref_df (pos>neg),
        between_lowpref_df  (pos<neg)
        """
        long_df = _to_long(df)
        pos_df = long_df[long_df["em"] == 1].copy()
        neg_df = long_df[long_df["em"] == 0].copy()

        highpref_list = _sample_between_pairs(pos_df, neg_df, n_per_set, relation="gt", seed=seed)
        lowpref_list  = _sample_between_pairs(pos_df, neg_df, n_per_set, relation="lt", seed=seed + 1)

        return pd.DataFrame(highpref_list), pd.DataFrame(lowpref_list)

    # ---------------------------------------------------------------------
    # Convenience wrapper to build everything at once
    # ---------------------------------------------------------------------
    def build_all_preference_sets(df: pd.DataFrame, n_between_per_set: int, seed: int = 42):
        """
        Returns a dict of four DataFrames:
        - within_lowpref:  same-qid, conf(pos) < conf(neg)
        - within_highpref: same-qid, conf(pos) > conf(neg)  (downsampled to match lowpref size)
        - between_highpref: cross-qid, conf(pos) > conf(neg), size ~ n_between_per_set
        - between_lowpref:  cross-qid, conf(pos) < conf(neg), size ~ n_between_per_set
        """
        within_lowpref, within_highpref = build_within_pairs(df, seed=seed)
        between_highpref, between_lowpref = build_between_pairs(df, n_per_set=n_between_per_set, seed=seed)
        return {
            "within_lowpref": within_lowpref,
            "within_highpref": within_highpref,
            "between_highpref": between_highpref,
            "between_lowpref": between_lowpref,
        }

    def _pack_sample(row: pd.Series):
        """
        Pack a single row into the requested tuple format:
        (qid, query, answer, em, method, conf)
        Note: JSON has no tuples; they'll be written as arrays.
        """
        return (row["qid"], row["query"], row["answer"], int(row["em"]), row["method"], float(row["conf"]))

    def _row_to_record(row: pd.Series, pid: int) -> dict:
        """Turn a paired row into a schema record with pid."""
        pos = {
            "qid": row["qid_pos"], "query": row["query_pos"], "answer": row["answer_pos"],
            "em": int(row["em_pos"]), "method": row["method_pos"], "conf": float(row["conf_pos"])
        }
        neg = {
            "qid": row["qid_neg"], "query": row["query_neg"], "answer": row["answer_neg"],
            "em": int(row["em_neg"]), "method": row["method_neg"], "conf": float(row["conf_neg"])
        }
        return {
            "pid": pid,
            "pair_type": row.get("pair_type", None),
            "positive_sample": _pack_sample(pd.Series(pos)),
            "negative_sample": _pack_sample(pd.Series(neg)),
        }

    def build_preference_df(sets: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        """
        Input: dict with keys like:
        "within_lowpref", "within_highpref", "between_highpref", "between_lowpref"
        Each value is a DataFrame produced by the earlier functions.
        Output: DataFrame with columns:
        ["pid", "pair_type", "positive_sample", "negative_sample"]
        """
        records: List[dict] = []
        pid = 0

        for key in ["within_lowpref", "within_highpref", "between_highpref", "between_lowpref"]:
            df_set = sets.get(key, None)
            if df_set is None or df_set.empty:
                continue

            # Ensure needed columns exist (in case of manual construction)
            expected_cols = {
                "qid_pos","query_pos","answer_pos","em_pos","method_pos","conf_pos",
                "qid_neg","query_neg","answer_neg","em_neg","method_neg","conf_neg",
                "pair_type"
            }
            missing = expected_cols - set(df_set.columns)
            if missing:
                raise ValueError(f"Set '{key}' is missing columns: {sorted(missing)}")

            for _, row in df_set.iterrows():
                records.append(_row_to_record(row, pid))
                pid += 1

        preference_df = pd.DataFrame.from_records(
            records,
            columns=["pid", "pair_type", "positive_sample", "negative_sample"]
        )
    ...
    # ---------------------------------------------------------------------
    # Example usage:
    sets = build_all_preference_sets(merged_df, n_between_per_set=3000, seed=args.seed)
    preference_df = build_preference_df(sets)
    preference_df.to_json(output_path, orient="records", lines=True, force_ascii=False)
    print(f"Saved preference dataset to {output_path}")
